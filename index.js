const authConfig = {
  ...
  "tokens":[
    {
	    "expires":0,
	    "refresh_token": "" // Authorization token 1
    },
    {
	    "expires":0,
	    "refresh_token": "" // Authorization token 2
    },
  ],
  /**
   ...
   * 申请refresh_token的链接，可以修改链接里面的client_id参数来申请其他client_id的token
   * 如果出现空白或者无法访问，可以试试修改下面包含'yanzai/goindex@${authConfig.version}'的地方，需要fork到自己在github上fork一个仓库
   */
  /**
  ...
  "roots": [
    {
	  tid: "0",	//对应refresh_token的索引
      id: "root",
      name: "个人盘1",
      // 只设置密码、只设置用户名、同时设置用户名密码，都是可以的，不设置的留空就行
      user: '',
      pass: '',
    },    
    {
...
class googleDrive {
   ...
   */
  async init() {
    await this.accessToken(this.authConfig.roots[this.order].tid);
    /*await (async () => {
        // 只获取1次
  ...
  async accessToken() {
    //console.log("accessToken:", this.order,this.root.tid);
    if (this.authConfig.tokens[this.root.tid].expires == undefined || this.authConfig.tokens[this.root.tid].expires < Date.now()) {
      const obj = await this.fetchAccessToken();
      if (obj.access_token != undefined) {
        this.authConfig.tokens[this.root.tid].accessToken = obj.access_token;
        this.authConfig.tokens[this.root.tid].expires = Date.now() + 3500 * 1000;
      }
    }
    return this.authConfig.tokens[this.root.tid].accessToken;
  }
  ...
  async fetchAccessToken() {
    ...
    const post_data = {
      ...
      'refresh_token': this.authConfig.tokens[this.root.tid].refresh_token,
